...
Generates a plot for each metric (EM, precision, recall, f1) with the number of solutions on the x-axis and the metric on the y-axis.
...
for metric in ['EM', 'precision', 'recall', 'f1']:
    ...
    plt.ylabel(metric)
    plt.tight_layout()
    fig_path = os.path.join(figures_dir, f'solutions-{metric}-{depth=}-{size=}.png')

...
Generates a plot for each metric (EM, precision, recall, f1) with aggregation (0/1) on the x-axis and the
...
for metric in ["EM", "precision", "recall", "f1"]:
    ...
    for i, row in df_mean.iterrows():
        ...
        plt.plot(
            x, y, label=i, marker="o", color=plotting_utils.COLORS[i], linestyle=plotting_utils.LINESTYLES[i]
        )

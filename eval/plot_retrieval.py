...
for metric in METRICS:
            ...
            for model_name, row in df_mean.iterrows():
                ...
                if model_name == 'gpt-4o-2024-11-20' and method_ in ['zeroshot-rag', 'cot-rag', 'react']:
                    ...
                    axs[i].plot(
                        ...
                        linestyle=plotting_utils.METHOD_LINESTYLES[method_], # NOTE: determine the linestyle using the method
                        linewidth=1,
                ...
                axs[i].fill_between(log10x, y-yerr, y+yerr, alpha=0.1, color=plotting_utils.COLORS[model_name])
                # Add scatter plot
                axs[i].scatter(
                    log10x, y,
                    color=plotting_utils.COLORS[model_name],
    ...
    for i in range(3):
        ...
        axs[i].set_xticklabels([f"$10^{int(np.log10(x))}$" for x in xticks if np.log10(x).is_integer()], fontsize=plotting_utils.TICK_FONT_SIZE)
        # set the xlim

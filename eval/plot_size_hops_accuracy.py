...
Generates a plot with the universe size on the x-axis, number of hops on the y-axis, and accuracy as the
contour.
...
parser = get_parser()
parser.add_argument(
    "--threshold_list", type=float, default=[0.25, 0.5, 0.75], nargs="+", help="List of thresholds to plot"
)
        ...
        for model in models:
            ...
            for i, size in enumerate(x):
                for j, hops in enumerate(y):
                    Z[j, i] = acc_mean_std_data_seed[
                        (acc_mean_std_data_seed["_size"].astype(int) == size)
                        & (acc_mean_std_data_seed["hops"] == hops)
                    ][(metric, "mean")].values[0]
            # filled in contour

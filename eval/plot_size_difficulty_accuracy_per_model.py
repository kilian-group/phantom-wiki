...
grouped = df.groupby(['_model', '_split', '_seed', 'difficulty'])
...
acc_mean_std = acc.groupby(['_model', '_split', 'difficulty']).agg(['mean', 'std'])
...
for data_seed in data_seeds:
    for metric in ['EM', 'precision', 'recall', 'f1']:
        for model in models:
            ...
            fig.tight_layout()
            fig_path = os.path.join(figures_dir, f"size-difficulty-{metric}-seed{data_seed}-{model.replace('/', '--')}.png")

#!/bin/bash
#SBATCH -J zeroshot-sc-large                              # Job name
#SBATCH -o slurm/zeroshot-sc-large_%j.out                 # output file (%j expands to jobID)
#SBATCH -e slurm/zeroshot-sc-large_%j.err                 # error log file (%j expands to jobID)
...
# if running on G2:
# sbatch --gres=gpu:a6000:8 --partition=kilian -t infinite eval/zeroshot-sc_L.sh <output directory>
# if running on empire:
# sbatch --gres=gpu:4 --partition=cornell -t 1-00:00:00 eval/zeroshot-sc_L.sh <output directory>

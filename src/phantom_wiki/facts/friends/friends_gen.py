...
def create_friendship_tree(names, number_of_communities=5, friendship_threshold=.7):
    """ 
        Given the names, this creates a friendship tree with `number_of_communities`
    ...
    for i in range(len(names)):
        for j in range(i+1, len(names)):
            if are_friends(individual_features[i], individual_features[j], friendship_threshold):
                output.append(f"friend({names[i]}, {names[j]}).")

    return output, individual_features
...
# Get prolog friendship tree and save to tests
pl_friendship_tree, coords = create_friendship_tree(names, friendship_threshold=20)
with open("tests/friendship_tree_100.pl", "w") as f_pl:
    f_pl.write("\n".join(pl_friendship_tree))

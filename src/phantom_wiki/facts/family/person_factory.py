...
@dataclass
class Career_Event:
    """A class representing career events of a person."""

    type: str  # Can be either 'start_job' or 'end_job'
    job: str
    date: str
...
class PersonFactory:
    ...
    def create_parents(self, tree_level: int, children: Person) -> list[Person]:
        ...
        out = [
            self.create_person(tree_level, parent_dob_1, False),
    ...
    def create_person(
        ...
        return Person(
            index=self._person_counter, name=name, female=female, tree_level=tree_level, date_of_birth=dob
        )

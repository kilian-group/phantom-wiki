...
class LLMChat(abc.ABC):
    ...
    @abc.abstractmethod
    def generate_response(self, conv: Conversation, stop_sequences: list[str] | None = None, seed: int = 1) -> str:
        """
    ...
    @abc.abstractmethod
    async def batch_generate_response(self, convs: list[Conversation], stop_sequences: list[str] | None = None, seed: int = 1) -> list[str]:
        """
        ...
        Returns:
            list[str]: List of responses generated by the model, one for each conversation.
        """
        pass
...
class CommonLLMChat(LLMChat):
    ...
    def _call_api(
        ...
        Use `convert_conv_to_api_format` to convert a conversation of type `Conversation` into such a list.
    ...
    async def _async_chat_completion(self, formatted_convs: list[list[dict]], stop_sequences: list[str] | None = None, seed: int = 1):
        """
    ...
    async def batch_generate_response(self, convs: list[Conversation], stop_sequences: list[str] | None = None, seed: int = 1) -> list[str]:
        """
        Generate responses for a batch of conversations using asynchronous API calls.
        
        Args:
            convs (list[Conversation]): List of conversations.
            stop_sequences (Optional[list[str]]): List of stop words to pause generating. Used if the API supports it.
                Defaults to None.
            seed (int): Seed for random number generator, passed to the model if applicable.
                Defaults to 1.
        ...
        responses = await self._async_chat_completion(formatted_convs, stop_sequences=stop_sequences, seed=seed)
        responses = [

    ...
    def _update_rate_limits(self, usage_tier: int) -> None:
        ...
        if usage_tier_str not in self.RATE_LIMITS[self.model_name]:
            raise ValueError(
                ...
                "please update the class for {self.model_name}."
            )
...
class TogetherChat(CommonLLMChat):
    ...
    RATE_LIMITS = {
        llm_name: {"usage_tier=0": {"RPM": 20, "TPM": 500_000}, "usage_tier=1": {"RPM": 600, "TPM": 500_000}}
...
class GeminiChat(CommonLLMChat):
    ...
    def _parse_api_output(self, response: object) -> LLMChatResponse:
        ...
        # "The candidate's [finish_reason](https://ai.google.dev/api/generate-content#finishreason)
        # is 4. Meaning that the model was reciting from copyrighted material."
        try:
            pred = response.text
            error = None
        ...
        if self.use_api:
            ...
            try:
                ...
                logging.error(
                    ...
                    f"--tensor_parallel_size NUM_GPUS --port {port}"
                )
                raise e
        else:
            ...
            if tensor_parallel_size is None:
                # NOTE: the reason why we can't use torch.cuda.device_count()
                # is because of some weird bug between torch and vllm,
